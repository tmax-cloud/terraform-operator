...
func (r *ProviderReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	*/
	...
	//if _, err := os.Stat(terraDir); os.IsNotExist(err) {
	//	err = os.Mkdir(terraDir, 0755)
	//	if err != nil {
	//		log.Error(err, "Failed to Create Terraform Working Directory")
	//		return ctrl.Result{}, err
	//	}
	//}
	...
	//		lines[i] = strings.Replace(lines[i], "{{ACCESS_KEY}}", providerAK, -1)
	//	}
	//	if strings.Contains(line, "{{SECRET_KEY}}") {
	//		lines[i] = strings.Replace(lines[i], "{{SECRET_KEY}}", providerSK, -1)
	//	}
	...
	/*
		...
		err = util.InitTerraform(terraDir, providerCloud)

		if err != nil {
			log.Error(err, "Terraform Init Error")
			return ctrl.Result{}, err
		}

		// Provision the resource provisioning by Terraform CLI
		isDestroy := false
		err = util.ExecuteTerraform(terraDir, isDestroy)
		...
		if err != nil {
			...
			tErr := r.Status().Update(ctx, provider)
			if tErr != nil {
				log.Error(tErr, "Failed to update Provider Status")
				return ctrl.Result{}, tErr
			}
		}
	*/
	/*
		...
		err = util.ExecuteTerraform(providerAK, providerSK)
		if err != nil {
			log.Error(err, "Terraform Error")
			return ctrl.Result{}, err
		}
	*/
	return ctrl.Result{}, nil
